...
export default function App() {
  ...
  const renderChar = (ch: string, i: number) => {
    ...
    const base = isTyped ? 'text-green-600' : isCurrent ? 'underline decoration-black animate-blinking ' + (ghostVisible ? 'text-black-400' : 'text-transparent') : ghostVisible ? 'text-gray-400' : 'hidden'
    const visible = ch === '\n' ? '↵' : ch === ' ' ? '·' : ch
    return (
